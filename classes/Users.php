...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			$qry1 = $this->conn->query("UPDATE users set amount = 0.00, usercode= '{$ucode}', refcode= '{$ref}' where id = '{$last_id}'");
			if($qry){
	...
	public function register_users(){
		...
		$sql = $this->conn->prepare("INSERT into users set amount = 0.00, role =?, type=?, password= ?, username= ?, parentid= ?, firstname= ?, middlename= ?, lastname= ? ");
