				...
				<tbody>
                    ...
                    <?php
                        ...
                        if($qryBeginingbalance ->num_rows > 0){
                            ...
                            $bal = $bal + $ending_amount;
                        }
                        ?>
						<tr>
							<td class=""><?php echo $i++; ?></td>
                            <td><span class="badge"><?php echo date("m-d-Y H:i", strtotime($ending_asof)) ?></span></td>
                            <td><span class="badge"><?php echo $type ?></span></td>
                            <td><?php echo number_format(0,2) ?> </td>
                            <td><?php echo number_format(0,2) ?> </td>
                            <td><?php echo number_format(0,2) ?> </td>
                            <td><span class="badge badge-light">N/A</span></td>
                            <td><?php echo number_format($ending_amount,2) ?> </td>
                            <td><?php echo number_format($ending_amount,2) ?> </td>
					...
					$i = 1;
                        ...
                        $qry = $conn->query("				select date_created,amount,0.00 red_amount,0.00 blue_amount,0.00 yellow_amount,1 as type,'' winner,'' drawno from loading a where active ='N' and user_id = '{$_settings->userdata('id')}'
									...
									(select winner from draws where id = a.drawid) as winner,(select concat(a.drawno,' - ',b.name) from draws a left join events b on b.id = a.eventid where a.id = a.drawid) as drawno
					...
					?>
						<tr>
							<td class=""><?php echo $i++; ?></td>
								...
								<td>
								<?php if($row['winner'] == 1): ?>
									<span class="badge badge-danger">Meron/Pula</span>
								<?php elseif($row['winner'] == 2): ?>
									<span class="badge badge-primary">Wala/Asul</span>
