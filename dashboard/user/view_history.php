			...
			if($qryBeginingbalance ->num_rows > 0){
				...
				$type = 'Unknown';
                        ...
                        $qry = $conn->query(" select '{$ending_asof}' date_created, '{$ending_amount}' amount, 0 as type, '' accttyp, '' processby
									...
									from users where id = a.agent_id) accttyp,
									(select username from users where id = a.agent_id) processby from coms_converted a where a.user_id = '{$_GET['id']}' and a.date_created >= '{$ending_asof}'
					...
					?>
						<tr>
							<td class=""><?php echo $i++; ?></td>
                            				<td><span class="badge"><?php echo date("m-d-Y H:i:s", strtotime($row['date_created'])) ?></span></td>
							<?php if($row['type'] == 2): ?>
